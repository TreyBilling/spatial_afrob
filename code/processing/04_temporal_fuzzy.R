...
# assign crs to each spatial window
...
# Subset to only no doubters
gtd_sf2 <- filter(gtd_sf, is.na(alternative))
...
gtd_sf2 <- st_transform(gtd_sf2, "+proj=aea +lat_1=20 +lat_2=-23 +lat_0=0 +lon_0=25 +x_0=0 +y_0=0 +ellps=WGS84 +datum=WGS84 +units=m no_defs")

# Make sure projections align
ggplot() +
  geom_sf(data = afrob_sf_buffers[[1]], color = "steelblue3") +
  geom_sf(data = gtd_sf, color = "red")
# Good
...
# Do the same, but filter to only attacks within a country's borders
gtd_buffers_within <- 
  map(.x = afrob_sf_buffers2, ~st_join(gtd_sf, .x) %>% 
        st_drop_geometry %>% 
        filter(!is.na(geoid) & NAME_0.x == NAME_0.y) %>% 
        dplyr::select(NAME_0 = NAME_0.x, everything(), -NAME_0.y) %>% 
        setDT())

# Create data.table for month lag indicators
# Only need 1 so grabbing ...[[1]]
...
# Within country
afrob_lag3_within <- imap(.x = gtd_buffers_within, 
                          ~.x[afrob.dt, on = .(geoid, gtd_date < date2, gtd_date >= lag_3)] %>% 
                            filter(!is.na(eventid)) %>% 
                            group_by(geoid, date) %>% 
                            tally(name = "attacks_within") %>%
                            full_join(afrob_sf_buffers[[1]], by = c("geoid", "date")) %>% 
                            mutate(temporal_window = '3 months',
                                   spatial_window = .y,
                                   attacks_within = ifelse(is.na(attacks_within), 0, attacks_within)))

afrob_lag6_within <- imap(.x = gtd_buffers_within, 
                          ~.x[afrob.dt, on = .(geoid, gtd_date < date2, gtd_date >= lag_6)] %>% 
                            filter(!is.na(eventid)) %>% 
                            group_by(geoid, date) %>% 
                            tally(name = "attacks_within") %>%
...
afrob_lag12_within <- imap(.x = gtd_buffers_within, 
                           ~.x[afrob.dt, on = .(geoid, gtd_date < date2, gtd_date >= lag_12)] %>% 
                             filter(!is.na(eventid)) %>% 
                             group_by(geoid, date) %>% 
                             tally(name = "attacks_within") %>%
...
# Within country
attacks_buff_within <- bind_rows(afrob_lag3_within, 
                                 afrob_lag6_within, 
                                 afrob_lag12_within) %>% 
  # Cleanup the naming of the spatial and temporal identifiers
  mutate(spatial_window = forcats::fct_inorder(case_when(spatial_window == 1 ~ "10km",
...
rm(afrob_lag3_within, afrob_lag6_within, afrob_lag12_within)


# Save as rdata, close R and reopen to avoid ram issues
# Continue at next chunk
save(attacks_buff, file = here::here("processed", "attacks_buff.Rdata"))
save(attacks_buff_within, file = here::here("processed", "attacks_buff_within.Rdata"))
...
load(here::here("processed", "attacks_buff.Rdata"))
...
temp <- 
  ...
  mutate(
    ...
    future_attacks3_months_10km = case_when(attacks3_months_10km == 0 & attacks99_months_10km > 0 ~ 1,
                                            TRUE ~ 0),
    future_attacks6_months_10km = case_when(attacks6_months_10km == 0 & attacks99_months_10km > 0 ~ 1,
                                            TRUE ~ 0),
    future_attacks12_months_10km = case_when(attacks12_months_10km == 0 & attacks99_months_10km > 0 ~ 1,
                                             TRUE ~ 0),
    
    
    future_attacks3_months_25km = case_when(attacks3_months_25km == 0 & attacks99_months_25km > 0 ~ 1,
                                            TRUE ~ 0),
    future_attacks6_months_25km = case_when(attacks6_months_25km == 0 & attacks99_months_25km > 0 ~ 1,
                                            TRUE ~ 0),
    future_attacks12_months_25km = case_when(attacks12_months_25km == 0 & attacks99_months_25km > 0 ~ 1,
                                             TRUE ~ 0),
    
    future_attacks3_months_50km = case_when(attacks3_months_50km == 0 & attacks99_months_50km > 0 ~ 1,
                                            TRUE ~ 0),
    future_attacks6_months_50km = case_when(attacks6_months_50km == 0 & attacks99_months_50km > 0 ~ 1,
                                            TRUE ~ 0),
    future_attacks12_months_50km = case_when(attacks12_months_50km == 0 & attacks99_months_50km > 0 ~ 1,
                                             TRUE ~ 0),
    
    
    future_attacks3_months_100km = case_when(attacks3_months_100km == 0 & attacks99_months_100km > 0 ~ 1,
                                             TRUE ~ 0),
    future_attacks6_months_100km = case_when(attacks6_months_100km == 0 & attacks99_months_100km > 0 ~ 1,
                                             TRUE ~ 0),
    future_attacks12_months_100km = case_when(attacks12_months_100km == 0 & attacks99_months_100km > 0 ~ 1,
                                              TRUE ~ 0))


afrob_tidy2 <- temp %>% full_join(afrob_merge_sf, by = c("geoid", "date"))

save(afrob_tidy, file = here::here("processed", "afrob_tidy.Rdata"))

save(afrob_tidy2, file = here::here("processed", "afrob_tidy2.Rdata"))
